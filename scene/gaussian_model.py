...
class GaussianModel:
    ...
    def sync_gradients_for_replicated_3dgs_storage(self, batched_screenspace_pkg):
        ...
        if "visible_count" in args.grad_normalization_mode:
            ...
            batched_locally_preprocessed_visibility_filter_int = [
                x.int()
                for x in batched_screenspace_pkg["batched_locally_preprocessed_visibility_filter"]
            ]
    ...
    def save_ply(self, path):   # here, we should be in torch.no_grad() context. train.py ensures that.
        ...
        if args.gaussians_distribution and not args.distributed_save:
            ...
            def gather_uneven_tensors(tensor):
                ...
                return gathered_tensors if group.rank() == 0 else None
        ...
        f_dc = _features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()    # N 1 3 ==> N 3 1 ==> N 3
        f_rest = _features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()    # N 15 3 ==> N 3 15 ==> N 45
    ...
    def distributed_load_ply(self, folder):
        ...
        self._features_dc = nn.Parameter(torch.tensor(catted_features_dc, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))  # N 1 3
        self._features_rest = nn.Parameter(torch.tensor(catted_features_rest, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))  # N (SH_degree**2 - 1) 3
    ...
    def load_raw_ply(self, path):
        ...
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))    # N 3 (SH_coeffs - 1)
            ...
            if stored_state is not None:
                if 'exp_avg' not in stored_state:
                    stored_state['momentum_buffer'] = torch.cat(
                        (stored_state['momentum_buffer'], torch.zeros_like(extension_tensor)), dim=0
                    )
                else:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(
            new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_send_to_gpui_cnt
        )
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(
            new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_send_to_gpui_cnt
        )
    ...
    def all2all_gaussian_state(self, state, destination, i2j_send_size):
        ...
        for j in range(comm_group.size()):  # ugly implementation.
            ...
            state_from_gpuj.append(
                torch.zeros(
                    (i2j_send_size[j][comm_group.rank()], *state.shape[1:]), device="cuda"
                )
            )
